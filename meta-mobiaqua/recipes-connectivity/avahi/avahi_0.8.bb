...
# MobiAqua: added 'skip-gobject-introspection.patch'
...
FILESEXTRAPATHS:prepend := "${THISDIR}/../../../meta/recipes-connectivity/avahi/files:${THISDIR}/files:"

# MobiAqua: removed "gobject-introspection", added "update-rc.d"
inherit autotools pkgconfig gettext github-releases update-rc.d

# MobiAqua: added "--disable-gobject", replaced with root: "--with-avahi-priv-access-group=adm", added: "--with-avahi-user=root", --with-avahi-group=root
EXTRA_OECONF = "--with-avahi-priv-access-group=root \
...
# MobiAqua: added initscript files
do_install:append() {
    if ${@bb.utils.contains('VIRTUAL-RUNTIME_init_manager','busybox','true','false',d)}; then
        install -d ${D}${sysconfdir}/init.d
        install -d ${D}${sysconfdir}/rcS.d
        install -m 0755 ${WORKDIR}/avahi-daemon ${D}${sysconfdir}/init.d/avahi-daemon
